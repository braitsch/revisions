package view.layout {
	...
	public class NestedList extends SimpleList {	
		...
		private function onToggleSubItems(e:UICommand):void 
		{
			...
			var n:uint = super.getChildIndex(super.activeItem);
		...
		private function insertIntoList(n:uint, d:Vector.<ListItem>, h:Number):void 
		{
			for (var i : int = 0; i < d.length; i++) {
				...
				super.addChildAt(d[i], n+i+1);
			}			
			for (var j : int = n+i+1; j < super.numChildren; j++) {
				var k:ListItem = super.getChildAt(j) as ListItem;
				k.y+=(h*d.length);
			}
		}		
		...
		{	
			...
			for (var i : int = n+1; i < super.numChildren; i++) {
				var k:ListItem = super.getChildAt(i) as ListItem;
				if (k.x <= super.activeItem.x) break;
				d.push(k);
			}			
			for (var j : int = 0; j < d.length; j++) super.removeChild(d[j]);
			for (var w : int = n+1; w < super.numChildren; w++) {
				var o:ListItem = super.getChildAt(w) as ListItem;
				o.y-=(h*d.length);
			}				
		}
