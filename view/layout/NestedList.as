package view.layout {
	...
	public class NestedList extends SimpleList {	
		...
		private function onToggleSubItems(e:UICommand):void 
		{
			...
			var n:uint = super.list.getChildIndex(super.activeItem);
		...
		private function insertIntoList(n:uint, d:Vector.<ListItem>, h:Number):void 
		{
			for (var i : int = 0; i < d.length; i++) {
				...
				super.list.addChildAt(d[i], n+i+1);
			}			
			for (var j : int = n+i+1; j < super.list.numChildren; j++) {
				var k:ListItem = super.list.getChildAt(j) as ListItem;
				k.y+=(h*d.length);
			}
		}		
		...
		{	
			...
			for (var i : int = n+1; i < super.list.numChildren; i++) {
				var k:ListItem = super.list.getChildAt(i) as ListItem;
				if (k.x <= super.activeItem.x) break;
				d.push(k);
			}			
			for (var j : int = 0; j < d.length; j++) super.list.removeChild(d[j]);
			for (var w : int = n+1; w < super.list.numChildren; w++) {
				var o:ListItem = super.list.getChildAt(w) as ListItem;
				o.y-=(h*d.length);
			}				
		}
